...
for (const [name, url, extract] of probes) {
  ...
  try {
    ...
    if (extract && response.ok) shape = extract(JSON.parse(raw));
      ...
      nextShape: name === 'NBA.com news HTML' ? (() => { try { const text = raw.match(/<script\b[^>]*id=["']__NEXT_DATA__["'][^>]*>([\s\S]*?)<\/script>/i)?.[1]; const props = JSON.parse(text).props?.pageProps || {}; return Object.fromEntries(['latest','features','serialized'].map(key => { const v = props[key]; const first = (Array.isArray(v) ? v : v?.items)?.[0] || {}; return [key, { length: (Array.isArray(v) ? v : v?.items)?.length, keys: Object.keys(first).slice(0, 30), sample: { title: first.title, date: first.date, timestamp: first.timestamp, permalink: first.permalink, status: first.status } }]; })); } catch { return {}; } })() : undefined,
      articleLd: name.includes('article') ? (() => { try { const tag = [...raw.matchAll(/<script\b[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)][0]?.[1]; const v = JSON.parse(tag); return { type: v['@type'], keys: Object.keys(v), headline: v.headline, url: v.url, datePublished: v.datePublished }; } catch { return {}; } })() : undefined,
