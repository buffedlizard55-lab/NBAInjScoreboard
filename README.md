...
**Latency targets, not SLAs:** hosted server polls live ESPN scoreboard about every 10s, summaries about every 6s + response time, injuries about every 12s, ESPN news about every 30s, NBA.com news (hosted mode) about every 45s with a 10-minute per-article refresh; the NBA PBP is attempted for mapped games. Browser fallback polls per tab (10s scoreboard, 8s summaries, 15s injuries, 30s news while games are live). Each provider controls its own publishing/caching latency; a 6s poll cannot make a 10-minute-late report real-time. Failed requests back off and the UI explicitly reports gaps. There are no background notifications when the page is closed; optional sound/desktop notifications require opt-in and browser permission and are suppressed for existing backfill.
